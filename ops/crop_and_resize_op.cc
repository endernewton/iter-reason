...
// car = tf.load_op_library('./crop_and_resize.so')
...
Status SetOutputToSizedImage(InferenceContext* c, DimensionHandle batch_dim, int size_input_idx, DimensionHandle channel_dim) {
...
inline void RunIfBoxIndexIsValid(OpKernelContext* context, typename TTypes<int32, 1>::ConstTensor box_index, int batch_size, const Callback& compute, const Callback& done);

// Specialization of CheckValidBoxIndex for a CPUDevice.
template <>
inline void RunIfBoxIndexIsValid<CPUDevice>(OpKernelContext* context, typename TTypes<int32, 1>::ConstTensor box_index, int batch_size, const Callback& compute, const Callback& done) {
...
REGISTER_OP("CropAndResize")
    ...
    .SetShapeFn([](InferenceContext* c) {
      ...
      return SetOutputToSizedImage(c, num_boxes_dim, 3, c->Dim(input, 3));
    });
...
class CropAndResizeOp : public AsyncOpKernel {
 public:
  explicit CropAndResizeOp(OpKernelConstruction* context) : AsyncOpKernel(context) {
    string method;
  ...
  void ComputeAsync(OpKernelContext* context, DoneCallback done) override {
    ...
    auto compute_callback = [this, context, output]() {
      ...
      const Tensor& boxes = context->input(1);
      const Tensor& box_index = context->input(2);
      const bool status = CropAndResize<Device, T>()(
...
struct CropAndResize<CPUDevice, T> {
  bool operator()(const OpKernelContext* context, typename TTypes<T, 4>::ConstTensor image, typename TTypes<float, 2>::ConstTensor boxes, typename TTypes<int32, 1>::ConstTensor box_index, float extrapolation_value, typename TTypes<float, 4>::Tensor crops) {
...
class CropAndResizeGradImageOp : public AsyncOpKernel {
 public:
  explicit CropAndResizeGradImageOp(OpKernelConstruction* context) : AsyncOpKernel(context) {
    string method;
    OP_REQUIRES_OK(context, context->GetAttr("method", &method));
    OP_REQUIRES(context, method == "bilinear",
                errors::InvalidArgument("method must be 'bilinear'", method));
  ...
  void ComputeAsync(OpKernelContext* context, DoneCallback done) override {
    ...
    auto compute_callback = [context, output]() {
      ...
      const Tensor& boxes = context->input(1);
      const Tensor& box_index = context->input(2);
      const bool status = CropAndResizeBackpropImage<Device, T>()(
...
struct CropAndResizeBackpropImage<CPUDevice, T> {
  bool operator()(const CPUDevice& d, typename TTypes<float, 4>::ConstTensor grads, typename TTypes<float, 2>::ConstTensor boxes, typename TTypes<int32, 1>::ConstTensor box_index, typename TTypes<T, 4>::Tensor grads_image) {
...
class CropAndResizeGradBoxesOp : public AsyncOpKernel {
 public:
  explicit CropAndResizeGradBoxesOp(OpKernelConstruction* context) : AsyncOpKernel(context) {
    string method;
    OP_REQUIRES_OK(context, context->GetAttr("method", &method));
    OP_REQUIRES(context, method == "bilinear",
                errors::InvalidArgument("method must be 'bilinear'", method));
  ...
  void ComputeAsync(OpKernelContext* context, DoneCallback done) override {
    ...
    auto compute_callback = [context, output]() {
      ...
      const Tensor& boxes = context->input(2);
      const Tensor& box_index = context->input(3);
      const bool status = CropAndResizeBackpropBoxes<Device, T>()(
          context->eigen_device<Device>(), grads.tensor<float, 4>(),
...
//   CheckValidBoxIndexHelper<GPUDevice>()(
